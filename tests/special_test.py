...
from mparray_test import MP_TEST_DTYPES

# nr_sites, local_dim, bond_dim
MP_BENCHMARK_PARAMETERS = [(10, 10, 5), (20, 2, 10)]


@pt.mark.long
@pt.mark.parametrize('dtype', MP_TEST_DTYPES)
@pt.mark.parametrize('nr_sites, local_dim, bond_dim', MP_BENCHMARK_PARAMETERS)
def test_inner_fast(nr_sites, local_dim, bond_dim, dtype, benchmark, rgen):
    ...
    res_fast = benchmark(mpsp.inner_prod_mps, mpa1, mpa2)
...
@pt.mark.parametrize('dtype', MP_TEST_DTYPES)
@pt.mark.parametrize('nr_sites, local_dim, bond_dim', MP_BENCHMARK_PARAMETERS)
def test_inner_slow(nr_sites, local_dim, bond_dim, dtype, benchmark, rgen):
    mpa1 = factory.random_mpa(nr_sites, local_dim, 1, dtype=dtype,
                              randstate=rgen)
    mpa2 = factory.random_mpa(nr_sites, local_dim, bond_dim, dtype=dtype,
                              randstate=rgen)
